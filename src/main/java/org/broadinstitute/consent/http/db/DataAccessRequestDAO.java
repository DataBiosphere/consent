...
/**
 * For all json queries, note the double `??` for jdbi3 escaped jsonb operators: <a
 * href="https://jdbi.org/#_postgresql">...</a>
 */
...
public interface DataAccessRequestDAO extends Transactional<DataAccessRequestDAO> {
  ...
  @SqlQuery(
      """
          ...
          """)
  List<DataAccessRequest> findAgedDARsByEmailTypeOlderThanInterval(
  ...
  @SqlQuery(
      """
          ...
          WHERE dar.reference_id IN (<referenceIds>)
            AND (LOWER(dar.data->>'status') != 'archived' OR dar.data->>'status' IS NULL)
      """)
  List<DataAccessRequest> findByReferenceIds(
  ...
  @SqlUpdate(
      """
        UPDATE data_access_request
          SET approving_so_id = :id, approving_so_timestamp = now()
        WHERE reference_id = :referenceId
    """)
  void updateDarApprovalSO(
  ...
  /**
   * Snapshots the institution a submission was validated against onto the DAR; null if there is
   * none or it no longer exists.
   *
   * @param referenceId The DAR or progress report reference id
   * @param institutionId The submitter's institution id, as validated
   */
  @SqlUpdate(
      """
        UPDATE data_access_request
          SET (institution_id, institution_name) =
              (SELECT institution_id, institution_name FROM institution
                WHERE institution_id = :institutionId),
            institution_snapshot_date = now()
        WHERE reference_id = :referenceId
        """)
  void updateSubmissionInstitution(
      @Bind("referenceId") String referenceId, @Bind("institutionId") Integer institutionId);
  ...
  List<Integer> findAllDARDatasetRelations(
      @BindList(value = "referenceIds", onEmpty = EmptyHandling.NULL_STRING)
          List<String> referenceIds);
}
