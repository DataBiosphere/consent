...
public interface DarCollectionDAO extends Transactional<DarCollectionDAO> {
    ...
    String getCollectionAndDars = " SELECT c.*, i.institution_name, u.display_name AS researcher, dd.dataset_id, " +
    ...
    @SqlQuery(getCollectionAndDars + " WHERE c.collection_id in (<collectionIds>)" + archiveFilterQuery)
    ...
    /**
     * Find the DARCollection and all of its Data Access Requests that contains the
     * DAR with the given
     * referenceId
     *
    ...
    /**
     * Finds a list of DarCollections (with all of their Data Access Requests) that
     * contain a DAR
     * whose referenceId is in the given list of referenceIds
     *
     * @return List of DarCollections associated with a referenceId in the given
     *         list
     */
    @RegisterBeanMapper(value = User.class, prefix = "u")
    @RegisterBeanMapper(value = Institution.class, prefix = "i")
    ...
    @SqlQuery("SELECT c.*, " +
            ...
            +
            "AND (LOWER(data->>'status')!='archived' OR data->>'status' IS NULL) ")
    List<DarCollection> findDARCollectionsByReferenceIds(
            @BindList("referenceIds") List<String> referenceIds);

    /**
     * Find the DARCollection and all of its Data Access Requests that has the given
     * collectionId
     *
     * @return DarCollection
     */
    ...
    /**
     * Create a new DAR Collection with the given dar code, create user ID, and
     * create date
     *
    ...
    /**
     * Update the update user and update date of the DAR Collection with the given
     * collection ID
     */
