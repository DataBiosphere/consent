...
public class DataAccessAgreementReducer
    implements LinkedHashMapRowReducer<Integer, DataAccessAgreement>,
        RowMapperHelper,
        ConsentLogger {
  ...
  public void accumulate(Map<Integer, DataAccessAgreement> map, RowView rowView) {
    var daaId =
        hasNonZeroColumn(rowView, "daa_id")
            ? rowView.getColumn("daa_id", Integer.class)
            : rowView.getColumn("daa_daa_id", Integer.class);
    map.computeIfAbsent(daaId, id -> rowView.getRow(DataAccessAgreement.class));
