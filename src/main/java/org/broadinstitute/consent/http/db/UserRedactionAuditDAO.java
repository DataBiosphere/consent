...
public interface UserRedactionAuditDAO {
   ...
   * FK migration) and the display_name with {@code 'redacted'}. The outer INSERT then writes the
   * audit row, selecting from the CTE so that if the UPDATE matches no rows the INSERT is also a
   * no-op.
   *
  ...
  @SqlUpdate(
      ...
      INSERT INTO user_redaction_audit
        (user_id, admin_user_id, original_email, original_display_name, action_date)
      SELECT :userId, :adminUserId, :originalEmail, :originalDisplayName, NOW()
      FROM update_user
      """)
  void redactUser(
