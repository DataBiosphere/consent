...
public class DraftReducer
    implements LinkedHashMapRowReducer<UUID, DraftInterface>, RowMapperHelper {

  @Override
  public void accumulate(Map<UUID, DraftInterface> map, RowView rowView) {
    DraftInterface draft =
        map.computeIfAbsent(
            rowView.getColumn("uuid", UUID.class), id -> rowView.getRow(DraftInterface.class));
    ...
    if (hasNonZeroColumn(rowView, "fso_file_storage_object_id")) {
      ...
      draft.addStoredFile(fileStorageObject);
    }
  }
}
