...
public interface ConsentDAO extends Transactional<ConsentDAO> {
    ...
    @SqlUpdate("insert into consents " +
            "(consentId, requiresManualReview, useRestriction, dataUse, dataUseLetter, active, name, dulName, createDate, sortDate, translatedUseRestriction, groupName) values " +
            "(:consentId, :requiresManualReview, :useRestriction, :dataUse, :dataUseLetter, true, :name , :dulName, :createDate, :sortDate , :translatedUseRestriction, :groupName)")
    ...
    @SqlQuery(
        "SELECT c.* FROM consents c "
            ...
            + " INNER JOIN dataset data ON data.dataset_id = ca.datasetid "
            + " WHERE c.consentId NOT IN "
                + "(SELECT c.consentId FROM consents c INNER JOIN election e ON e.referenceId = c.consentId )"
            + " AND data.dac_id IN (<dacIds>)")
    List<Consent> findUnreviewedConsentsForDacs(@BindList("dacIds") List<Integer> dacIds);
