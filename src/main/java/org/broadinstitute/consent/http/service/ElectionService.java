...
public class ElectionService {
  ...
  @Inject
  public ElectionService(ConsentDAO consentDAO, ElectionDAO electionDAO, VoteDAO voteDAO,
      UserDAO userDAO,
  ...
  public Election updateElectionById(Election rec, Integer electionId) {
    ...
    } else if (rec.getStatus().equals(ElectionStatus.CLOSED.getValue()) || rec.getStatus()
        .equals(ElectionStatus.FINAL.getValue())) {
    ...
    if (rec.getStatus().equals(ElectionStatus.CANCELED.getValue()) || rec.getStatus()
        .equals(ElectionStatus.CLOSED.getValue())) {
    ...
    updateSortDate(electionDAO.findElectionWithFinalVoteById(electionId).getReferenceId(),
        lastUpdate);
  ...
  public Election describeDataRequestElection(String requestId) {
    Election election = electionDAO.getElectionWithFinalVoteByReferenceIdAndType(requestId,
        ElectionType.DATA_ACCESS.getValue());
    if (election == null) {
      election = electionDAO.getElectionWithFinalVoteByReferenceIdAndType(requestId,
  ...
  /**
   * Return true if: There are no RP or DAR votes and the vote is not a chairperson vote All RP
   * chair votes have not been created All DAR chair votes have not been created
   *
  ...
  public boolean validateCollectDAREmailCondition(Vote vote) {
    ...
    List<Integer> chairIds = electionChairs.stream().map(User::getUserId)
        .collect(Collectors.toList());
    Integer exists = mailMessageDAO.existsCollectDAREmail(darReferenceId, rpReferenceId);
    if ((exists == null)) {
      if (((darVotes.size() == 0) && (rpElectionVotes.size() == 0) && (!chairIds.contains(
          vote.getUserId())))) {
        return true;
      } else {
        List<Vote> rpChairVotes = voteDAO.findVotesByElectionIdAndUserIds(rpElectionId, chairIds);
        List<Vote> darChairVotes = voteDAO.findVotesByElectionIdAndUserIds(vote.getElectionId(),
            chairIds);
  ...
  public void closeDataOwnerApprovalElection(Integer electionId) {
    ...
    List<Vote> dataOwnersVote = voteDAO.findVotesByElectionIdAndType(election.getElectionId(),
        VoteType.DATA_OWNER.getValue());
    List<Vote> rejectedVotes = dataOwnersVote.stream().filter(
        dov -> (dov.getVote() != null && !dov.getVote()) || (dov.getHasConcerns() != null
            && dov.getHasConcerns())).collect(Collectors.toList());
    ...
    electionDAO.updateElectionById(electionId, election.getStatus(), new Date(),
    ...
    try {
      ...
      if (validateAllDatasetElectionsAreClosed(dsElections)) {
        ...
        darElections.add(electionDAO.findLastElectionByReferenceIdAndType(election.getReferenceId(),
            ElectionType.DATA_ACCESS.getValue()));
  ...
  public boolean checkDataOwnerToCloseElection(Integer electionId) {
    ...
    if (election.getElectionType().equals(ElectionType.DATA_SET.getValue())) {
      List<Vote> pendingVotes = voteDAO.findDataOwnerPendingVotesByElectionId(electionId,
          VoteType.DATA_OWNER.getValue());
