...
public class SummaryService {
  ...
  private List<Summary> getMatchSummaryCases() {
    ...
    summaryList.add(createSummary(0, matchDAO.countMatchesByResult(Boolean.TRUE),
        matchDAO.countMatchesByResult(Boolean.FALSE)));

    List<Election> latestElections = electionDAO.findLastElectionsWithFinalVoteByType(
    ...
    if (!CollectionUtils.isEmpty(reviewedElections)) {
      ...
      if (CollectionUtils.isNotEmpty(agreementYesVotes) || CollectionUtils.isNotEmpty(
          agreementNoVotes)) {
  ...
  protected Summary getSummaryCases(String type) {
    ...
    Integer totalPositiveCases = electionDAO.findTotalElectionsByTypeStatusAndVote(type,
        ElectionStatus.CLOSED.getValue(), true);
    Integer totalNegativeCases = electionDAO.findTotalElectionsByTypeStatusAndVote(type,
        ElectionStatus.CLOSED.getValue(), false);
        ...
        for (Election election : reviewedElections) {
          ...
          Optional<User> chairPerson = chairPersonVote
              .flatMap(vote -> users.stream()
                  .filter(du -> du.getUserId().equals(vote.getUserId()))
                  .findFirst());
  ...
  /**
   * Generate a list of Summary Details for all reviewed Data Access Requests Requires a significant
   * amount of database interaction as we need to find all completed elections for DARs, datasets,
   * votes, matches, and users for those elections. Batch what we can as efficiently as possible and
   * calculate the rest.
   *
  ...
  public List<DataAccessRequestSummaryDetail> listDataAccessRequestSummaryDetails() {
    ...
    if (!accessElections.isEmpty()) {
      ...
      List<Vote> rpVotes = rpElectionIds.isEmpty() ? Collections.emptyList()
          : voteDAO.findVotesByElectionIds(rpElectionIds);
      List<Vote> consentVotes = consentElectionIds.isEmpty() ? Collections.emptyList()
          : voteDAO.findVotesByElectionIds(consentElectionIds);
      ...
      for (Election accessElection : accessElections) {
        ...
        if (Objects.nonNull(dar) && Objects.nonNull(dar.getData())) {
          ...
          if (CollectionUtils.isNotEmpty(datasetId)) {
            ...
            Optional<User> darUser = darUsers.stream()
                .filter(u -> u.getUserId().equals(dar.getUserId())).findFirst();
