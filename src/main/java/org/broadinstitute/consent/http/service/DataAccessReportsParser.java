...
public class DataAccessReportsParser implements ConsentLogger {
  ...
  public void setApprovedDARHeader(FileWriter darWriter) throws IOException {
    darWriter.write(
        HeaderDAR.DAR_ID.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.DATASET_NAME.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.DATASET_ID.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.CONSENT_ID.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.DATA_REQUESTER_NAME.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.ORGANIZATION.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.CODED_VERSION_SDUL.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.CODED_VERSION_DAR.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.RESEARCH_PURPOSE.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.DATE_REQUEST_SUBMISSION.getValue()
            + DEFAULT_SEPARATOR
            + HeaderDAR.DATE_REQUEST_APPROVAL.getValue()
            + DEFAULT_SEPARATOR
  ...
  public void setReviewedDARHeader(FileWriter darWriter) throws IOException {
    darWriter.write(
        HeaderDAR.DAR_ID.getValue()
            ...
            + DEFAULT_SEPARATOR
            + HeaderDAR.APPROVED_DISAPPROVED.getValue()
            + END_OF_LINE);
  }

  public void addApprovedDARLine(
      FileWriter darWriter,
      Election election,
    ...
    String electionDate =
        (Objects.nonNull(election.getFinalVoteDate()))
            ? formatTimeToDate(election.getFinalVoteDate().getTime())
            : "";
    String updateDate =
        (Objects.nonNull(dar.getUpdateDate()))
            ? formatTimeToDate(dar.getUpdateDate().getTime())
            : "";
    String content2 =
        rusSummary
            + DEFAULT_SEPARATOR
  ...
  public void addReviewedDARLine(
      FileWriter darWriter,
      Election election,
      DataAccessRequest dar,
      String darCode,
  ...
  private void addDARLine(
      FileWriter darWriter,
      DataAccessRequest dar,
      String darCode,
      String customContent1,
      String customContent2,
      String consentName,
      String translatedUseRestriction)
      throws IOException {
    ...
    if (Objects.nonNull(dar.getData())) {
      ...
      String sDUL =
          StringUtils.isNotEmpty(translatedUseRestriction)
      ...
      darWriter.write(
          darCode
              + DEFAULT_SEPARATOR
              + StringUtils.join(datasetNames, ",")
              + DEFAULT_SEPARATOR
              + StringUtils.join(datasetIdentifiers, ",")
              + DEFAULT_SEPARATOR
              + consentName
              + DEFAULT_SEPARATOR
              + customContent1
