 ...
 *
 * <p>Not serialized against DAR activity. {@code reprocessMatchesForPurpose} reads its DAR outside
 * the transaction it writes in, so a DAR edited mid-run can have its own rebuild replaced by this
 * one. That race predates this class and is left where it is rather than locked around: this is an
 * operator-triggered one-off, so a quiet window is the proportionate answer.
 */
