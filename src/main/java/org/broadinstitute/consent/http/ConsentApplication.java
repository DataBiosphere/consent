...
public class ConsentApplication extends Application<ConsentConfiguration> {
    ...
    public void run(ConsentConfiguration config, Environment env) {
        ...
        DatabaseAuditServiceAPI.initInstance(workspaceAuditDAO, userDAO, associationDAO);
        DatabaseDataAccessRequestAPI.initInstance(mongoInstance, useRestrictionConverter, electionDAO, consentDAO, voteDAO, userDAO, dataSetDAO, researcherPropertyDAO);
        ...
        DatabaseDataSetAssociationAPI.initInstance(dataSetDAO, dataSetAssociationDAO, userDAO);

        try {
            MailService.initInstance(config.getMailConfiguration());
            EmailNotifierService.initInstance(voteDAO, mongoInstance, electionDAO, userDAO, emailDAO, mailServiceDAO, new FreeMarkerTemplateHelper(config.getFreeMarkerConfiguration()), config.getServicesConfiguration().getLocalURL(), config.getMailConfiguration().isActivateEmailNotifications(), researcherPropertyDAO);
        } catch (IOException e) {
        ...
        DatabaseSummaryAPI.initInstance(voteDAO, electionDAO, userDAO, consentDAO, dataSetDAO ,matchDAO, mongoInstance, dataSetDAO);
        DatabaseElectionCaseAPI.initInstance(electionDAO, voteDAO, userDAO, userRoleDAO, consentDAO, mongoInstance, dataSetDAO);
        DACUserRolesHandler.initInstance(userDAO, userRoleDAO, electionDAO, voteDAO, dataSetAssociationDAO, AbstractEmailNotifierAPI.getInstance(), AbstractDataAccessRequestAPI.getInstance());
        DatabaseDACUserAPI.initInstance(userDAO, userRoleDAO, electionDAO, voteDAO, dataSetAssociationDAO, AbstractUserRolesHandler.getInstance(), researcherPropertyDAO);
        DatabaseVoteAPI.initInstance(voteDAO, userDAO, electionDAO, dataSetAssociationDAO);
        ...
        // Mail Services
        DatabaseElectionAPI.initInstance(electionDAO, consentDAO, userDAO, mongoInstance, voteDAO, emailDAO, dataSetDAO);
        ...
        final ResearcherAPI researcherAPI = new DatabaseResearcherAPI(researcherPropertyDAO, userDAO, AbstractEmailNotifierAPI.getInstance());
        ...
        env.jersey().register(new DataAccessRequestResource(DatabaseDACUserAPI.getInstance(), DatabaseElectionAPI.getInstance(), googleStore, userService));
        ...
        env.jersey().register(new DataRequestVoteResource(userService));
