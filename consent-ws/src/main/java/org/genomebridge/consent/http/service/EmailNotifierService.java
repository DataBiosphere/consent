...
public class EmailNotifierService extends AbstractEmailNotifierAPI {
    ...
    public void sendNewCaseMessage(Integer voteId) throws MessagingException, IOException, TemplateException {
        if(isServiceActive){
            Map<String, String> data = retrieveForVote(voteId);
            Writer template = templateHelper.getNewCaseTemplate(data.get("userName"), data.get("electionType"), data.get("entityId"), SERVERURL);
    ...
    @Override
    public void sendNewCaseMessageToList(List<Integer> votes) throws MessagingException, IOException, TemplateException {
        for(Integer voteId: votes){
            sendNewCaseMessage(voteId);
        }
    }
}
