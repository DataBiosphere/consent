...
public interface ElectionDAO extends Transactional<ElectionDAO> {
    ...
    @SqlQuery("SELECT v.electionId FROM vote v, election e where v.dacUserId = :dacUserId and e.electionId = v.electionId and v.vote is null AND e.electionType = '"
            + DATASET + "' AND e.status = '" + OPEN + "'")
    List<Integer> findDataSetOpenElectionIds(@Bind("dacUserId") Integer dacUserId);

    @SqlQuery("SELECT v.electionId FROM vote v, election e where v.dacUserId = :dacUserId and e.electionId = v.electionId AND v.vote is null AND e.electionType != '"
            + DATASET + "' AND (e.status = '" + OPEN + "' OR e.status = 'Final')")
    List<Integer> findNonDataSetOpenElectionIds(@Bind("dacUserId") Integer dacUserId);

    @SqlQuery("SELECT v.electionId FROM vote v, election e where v.dacUserId = :dacUserId and e.electionId = v.electionId AND v.vote is null AND e.electionType = :type AND (e.status = '"
            + OPEN + "' OR e.status = 'Final')")
