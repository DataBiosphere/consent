...
public class DatabaseConsentAPI extends AbstractConsentAPI {
    ...
    public Consent update(String id, Consent rec) throws NotFoundException {
        ...
        consentDAO.updateConsent(id, rec.getRequiresManualReview(), rec.getUseRestriction().toString(), rec.getDataUse().toString(), rec.getDataUseLetter(), rec.getName(), rec.getDulName(), rec.getLastUpdate(), rec.getSortDate(), rec.getTranslatedUseRestriction(), rec.getGroupName());
        return consentDAO.findConsentById(id);
    }
